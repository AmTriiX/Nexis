...
def chat(request):
    ...
    response = requests.get('http://127.0.0.1:5000/api_keys', params={'user_id': user_id})
...
def signup(request):
    ...
    if request.method == "POST":
        ...
        if username == 'nexis'.lower() and not request.user.is_superuser:
            messages.error(
                request, "Username already exist, Please try some other username.")
            return redirect('signup')
...
def my_account(request):
    if request.method == "POST":
        ...
        if pass1 == myuser.password:
            change_password(myuser, pass1)
